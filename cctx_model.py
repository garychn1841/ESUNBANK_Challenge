...
public_data = row_data.drop(['alert_key','cust_id','date'],axis = 1)
#data standarized
...
df_cctx.to_csv('cctx_final_public_proba.csv',index=False)

df_pred2 = pd.DataFrame(a3,columns=['1-probability','probability']).drop('1-probability',axis=1)
df_cctx2 = pd.concat([row_data,df_pred2],axis=1)
df_cctx2 = df_cctx2[['alert_key','probability']]
df_cctx2.to_csv('cctx2_final_public_proba.csv',index=False)

df_pred3 = pd.DataFrame(a3,columns=['1-probability','probability']).drop('1-probability',axis=1)
df_cctx3 = pd.concat([row_data,df_pred3],axis=1)
df_cctx3 = df_cctx3[['alert_key','probability']]
df_cctx3.to_csv('cctx3_final_public_proba.csv',index=False)