...
# df_GS.to_csv('GS_scroe.csv',index = False)

clf = svm.SVC(kernel='rbf' , gamma = 'auto',C=10 , probability=True,class_weight='balanced')
clf.fit(X_train_std,y_train)
...
df_ccba = pd.concat([row_data,df_pred],axis=1)
df_ccba = df_ccba[['alert_key','probability']]
df_ccba.to_csv('ccba_svm_final_public_proba.csv',index=False)

df_pred2 = pd.DataFrame(a2,columns=['1-probability','probability']).drop('1-probability',axis=1)
df_ccba2 = pd.concat([row_data,df_pred2],axis=1)
df_ccba2 = df_ccba2[['alert_key','probability']]
df_ccba2.to_csv('ccba2_final_public_proba.csv',index=False)

df_pred3 = pd.DataFrame(a3,columns=['1-probability','probability']).drop('1-probability',axis=1)
df_ccba3 = pd.concat([row_data,df_pred3],axis=1)
df_ccba3 = df_ccba3[['alert_key','probability']]
df_ccba3.to_csv('ccba3_final_public_proba.csv',index=False)